 ...
 */
export function enrichOffer(offer, allOffers = [], jobIndex = null, cityIndex = null) {
  ...
  try {
    ...
    if (jobMatch && jobWeights[jobMatch]) {
      ...
      enrichedOffer.enriched.related_jobs = relatedJobs
        ...
        .slice(0, 20)
        .map(rel => {
          // Lookup O(1) en índice pre-calculado
          const count = jobIndex ? (jobIndex.get(normalizeJobTitle(rel.job)) || 0) : 0;
  ...
  try {
    ...
    if (cityName && cityDistances[cityName]) {
      ...
      enrichedOffer.enriched.nearby_cities = nearbyCities
        ...
        .map(city => {
          // Lookup O(1) en índice pre-calculado
          const count = cityIndex ? (cityIndex.get(city.city) || 0) : 0;
...
export function enrichOffers(offers) {
  ...
  // Pre-calcular índices para optimizar available_offers (O(n) en lugar de O(n²))
  console.log('   📑 Creando índices de jobs y cities...');
  const jobIndex = new Map();
  const cityIndex = new Map();

  offers.forEach(offer => {
    // Índice de jobs
    const jobKey = normalizeJobTitle(offer.titulo || offer.title);
    if (jobKey) {
      jobIndex.set(jobKey, (jobIndex.get(jobKey) || 0) + 1);
    }

    // Índice de cities
    const cityKey = normalizeCityName(offer.ciudad || offer.city);
    if (cityKey) {
      cityIndex.set(cityKey, (cityIndex.get(cityKey) || 0) + 1);
    }
  });

  console.log(`   ✅ Índices creados: ${jobIndex.size} jobs, ${cityIndex.size} cities`);

  const enrichedOffers = offers.map(offer => enrichOffer(offer, offers, jobIndex, cityIndex));
